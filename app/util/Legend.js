...
Ext.define('CpsiMapview.util.Legend', {
    ...
    requires: ['BasiGX.util.Object', 'CpsiMapview.util.Html'],
    ...
    createGetLegendGraphicUrl: function (layer) {
        ...
        if (layer.get('isWms') || layer.get('isVt')) {
            ...
            if (layer.get('isVt')) {
                ...
                layers = BasiGX.util.Object.layersFromParams(
                    source.getParams()
                );
            }
            ...
            requestParams +=
                'FORMAT=image%2Fpng&TRANSPARENT=TRUE&SLD_VERSION=1.1.0&';
            ...
            url = layer.get('url');
            const ft = layer.get('featureType');
            activatedStyle = layer.get('activatedStyle');
            if (!url || !ft) {
                return;
            }
    ...
    getUniqueLayersParam: function (layers) {
        ...
        const reducedLayerList = Ext.Array.unique(layerList);
    ...
    getLegendImgHtmlTpl: function (legendUrl, width, height) {
        const ns = 'CpsiMapview.plugin.BasicTreeColumnLegends';
        return (
            '<img' +
    ...
    cacheLegendImgAsDataUrl: function (legendUrl, layerKey) {
        ...
        CpsiMapview.util.Html.addEvent(legendImage, 'load', function () {
            ...
            const legendDataUrl = canvas.toDataURL('image/png');
