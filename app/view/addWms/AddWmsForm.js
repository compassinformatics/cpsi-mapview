    ...
    viewModel: {
        data: {
            ...
            errorIncompatibleWMS:
                'The requested WMS is not compatible to the application',
            errorRequestFailed: 'The given URL could not be requested.',
            errorCouldntParseResponse:
                'The received response could not be parsed successfully.',
            msgRequestTimedOut:
                'The request was not answered in time and was aborted.',
            msgServiceException: 'An OGC ServiceException has occurred',
            msgCorsMisconfiguration:
                'HTTP access control (CORS) on the target server is probably not ' +
                'configured correctly.',
            msgUnauthorized:
                'The client is not authenticated against the target server.',
            ...
            msgServiceUnavailable:
                'The server is currently not available (too many or request or maintenance mode).',
            msgGatewayTimeOut:
                'The server acted as a gateway and the original target did not respond in time',
            msgClientError: 'An unspecified client error has occurred.',
            msgServerError: 'An unspecified server error has occurred.',
            documentation:
                '<h2>Add WMS</h2>• In this dialog you can add any desired map service to the map with the ' +
                'help of an WMS-URL.'
        }
    },
